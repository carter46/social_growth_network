...
<section class="relative w-full overflow-hidden bg-slate-900 text-white py-14 md:py-20">
    ...
    <div class="relative max-w-site mx-auto px-4 sm:px-6 lg:px-8 z-10">
        <div class="max-w-3xl flex flex-col items-start">
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 backdrop-blur-sm text-white text-[11px] font-bold tracking-wider uppercase mb-4">
                <span class="relative flex h-2 w-2">
                    <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-blue-300 opacity-75"></span>
                    <span class="relative inline-flex rounded-full h-2 w-2 bg-primary"></span>
                </span>
                Digital campaign marketplace
            </div>
            ...
            @if($popularTags->isNotEmpty())
                <div class="flex flex-wrap items-center gap-2 mt-4 text-sm text-slate-400">
                    <span class="text-slate-300">Popular:</span>
                    @foreach($popularTags as $tag)
                        <a
                            href="{{ route('services', ['q' => $tag['label'] ?? '']) }}"
                            class="px-2.5 py-1 bg-white/10 hover:bg-white/20 text-white rounded-lg transition-colors text-xs sm:text-sm"
                        >{{ $tag['label'] ?? '' }}</a>
                    @endforeach
                </div>
            @endif
...
<section class="w-full bg-white border-b border-slate-200 shadow-sm sticky top-24 z-30">
...
{{-- Main browsing area --}}
<section class="max-w-site mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10 w-full">
                ...
                <div>
                    ...
                    <p class="text-sm text-slate-500 mt-0.5">
                        ...
                        @elseif($activeCategory !== '')
                            @php $activeLabel = $groups->firstWhere('slug', $activeCategory)['label'] ?? $activeCategory; @endphp
                            Showing {{ $productCount }} {{ \Illuminate\Support\Str::plural('package', $productCount) }} in {{ $activeLabel }}
                        @else
                            Showing {{ $productCount }} active predefined {{ \Illuminate\Support\Str::plural('campaign', $productCount) }}
                        @endif
                    </p>
                </div>
                <form method="GET" action="{{ route('services') }}" class="flex items-center gap-2">
                    @if($q !== '')
                        <input type="hidden" name="q" value="{{ $q }}">
                    @endif
                    @if($activeCategory !== '')
